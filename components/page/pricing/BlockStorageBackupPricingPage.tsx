...
export default function BlockStorageBackupPricingPage() {
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        const res = await fetch(`/api/pricing/Block Storage Backup`);
        ...
        <div className="py-16 lg:py-32 md:py-32">
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="px-4 lg:px-8 md:px-8">
              <DataTable columns={vmSnapshotColumns("Block Storage Backup")} data={plans} />
            </div>
          </div>
        </div>
      </div>
