    ...
    <section className="max-w-7xl mx-auto  top-full left-0 flex flex-col gap-0 px-0 lg:px-4 lg:py-6 py-0">
      <section className="flex flex-col lg:flex-row lg:gap-8 gap-6">
        ...
        <section className="lg:h-84 h-fit w-full overflow-y-auto">
          <p className="font-semibold text-[24px] pb-4">Our Product Services</p>
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-2 gap-y-4 py-2 justify-between">
            {products.map((product: NavbarProducts) => (
              <a href={product.path} className="hover:text-gcxPrimary hover:cursor-pointer w-fit" key={product.id}>
                {product.name}
              </a>
            ))}
          </div>
        ...
        <section className="w-full lg:max-w-[400px] lg:block hidden">
          <FeatureCarouselComponent items={products} />
        </section>
      </section>
    </section>
  );
